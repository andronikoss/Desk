         ...
         ols = {
           draw.ols = function(obj, plot.what = "regline", ...){
             ...
             if (plot.what == "confint"){
               draw.confint = function(C, which.coef = 2, center = TRUE, center.size = 0.5,
                                       xlim, lwd = 1, ... ){
                 ...
                 clrs = ifelse(tr.par < lower | tr.par > upper, "black", "gray60")
         ...
         htest = {
           ...
           draw.ellipse = function (obj, sig.level = 0.05, type = "acceptance",
             ...
             if (center){
               ...
               text(center.loc[1], c(center.loc[2]), center.name, cex = 0.8, adj = c(1.2,-0.3))
               abline(h = center.loc[2], lty = 3, col = "gray70")
             }
             if (test.point){
               points(t(test.loc), col = "black", pch = 16)
               text(t(test.loc), test.name, cex = 0.8, adj = c(1.2,-0.3))
             }
           ...
           draw.test = function (obj, xlim = c(0,4), plot.what = "dist", xlab, ylab, legpos = "topright", ...){     
             ...
             # Welche Version des Buches wird compiliert?
             ...
             switch(plot.what,
                    ...
                    dist = {
                      ...
                      fill.area = function(a, b, points = 200, ...){
                        coord.x = c(a, seq(a, b, length = points), b) 
                        coord.y = c(0, call.dist(type = "d", dist = dist, value = seq(a, b, length = points), param = param), 0)
                      ...
                      # print(list(dist,param,tstat,cr.val, xlim))
                      if (missing(xlim)){
